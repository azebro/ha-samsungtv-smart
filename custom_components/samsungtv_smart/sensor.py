...
async def async_setup_entry(
    ...
    # Check if Frame TV is supported (flag set by __init__.py)
    frame_tv_supported = hass.data[DOMAIN][entry.entry_id].get(DATA_ART_API, False)

    if frame_tv_supported:
        # Each platform creates its own art_api to avoid concurrent WebSocket issues
        art_api = SamsungTVAsyncArt(
            ...
            session=session,
            timeout=5,
            name=f"{WS_PREFIX} {ws_name} Art Sensor",
        )
    ...
    # Add Recommended Art Brightness sensor if illuminance sensor configured
    options = hass.data[DOMAIN][entry.entry_id].get(DATA_OPTIONS, {})
    illuminance_sensor = options.get(CONF_ILLUMINANCE_SENSOR)
    if illuminance_sensor and frame_tv_supported:
        lux_config = {
            "min_lux": float(options.get(CONF_BRIGHTNESS_MIN_LUX, 1)),
            "max_lux": float(options.get(CONF_BRIGHTNESS_MAX_LUX, 1000)),
            "min_brightness": int(options.get(CONF_BRIGHTNESS_MIN, 5)),
            "max_brightness": int(options.get(CONF_BRIGHTNESS_MAX, 100)),
        }
        entities.append(
            FrameArtRecommendedBrightnessSensor(
                config=config,
                entry_id=entry.entry_id,
                illuminance_entity_id=illuminance_sensor,
                lux_config=lux_config,
            )
        )
        _LOGGER.info("Recommended Art Brightness sensor created for %s", device_name)
...
class FrameArtRecommendedBrightnessSensor(SamsungTVEntity, SensorEntity):
    ...
    def __init__(
        ...
        lux_config: dict[str, Any],
    ) -> None:
        """Initialize the recommended brightness sensor."""
        super().__init__(config, entry_id)
        self._attr_unique_id = f"{entry_id}_recommended_art_brightness"
    ...
    async def async_added_to_hass(self) -> None:
        ...
        # Set initial value from current illuminance reading
        current = self.hass.states.get(self._illuminance_entity_id)
        if current and current.state not in ("unknown", "unavailable"):
            try:
                lux = float(current.state)
                self._attr_native_value = lux_to_brightness(lux, **self._lux_config)
    ...
    def _illuminance_changed(self, event) -> None:
        ...
        # Only proceed when computed value actually changes
    ...
    @callback
    def _apply_brightness(self, brightness: int, _now) -> None:
